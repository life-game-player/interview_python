...
>思路:
>定义青蛙跳上一个n级台阶总共有f(n)种跳法。
>考察需要跳n级台阶时最后一跳，只存在以下两种情况：
>1） 跳1级台阶：前面跳了n-1级台阶，共f(n-1)种跳法
>2） 跳2级台阶：前面跳了n-2级台阶， 共f(n-2)种跳法
>所以，f(n) = f(n-1) + f(n-2)
...
>定义总共有f(n)种方法。
>分两种情况考虑：
>1) 第一次横着放小矩形，那么余下为面积为2*(n-1)的大矩形，故有f(n-1)种方法
>2) 第一次竖着放小矩形，那么必然要在其旁竖着放另一个小矩形，否则不可能拼出大矩形。这样一来余下为面积为2*(n-2)的大矩形，故有f(n-2)种方法
>综上，f(n) = f(n-1) + f(n-2)
